...
- Bulk files for historical backfill live in `backend/data/raw/<league>/` (gitignored).
  ...
  **derived**, exported from a geocoded dev database by
  ...
  `services/adapters/local_time.py` — see `docs/open_issues.md` #7.
  ...
  **noon** UTC via `local_time.date_only()`, never midnight — midnight renders as
  ...
  to UTC. Use the helper rather than restating the hour; see `open_issues.md` #8.
...
`docs/open_issues.md` #6). So a new database needs `alembic upgrade head` before
