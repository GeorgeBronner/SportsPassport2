...
- **Project Path**: `/home/ubuntu/cfb-tracker`
...
```bash
git push origin main && ssh oracle "cd cfb-tracker && git pull origin main && docker compose up -d --build"
```

**Step by step:**
1. Commit and push locally: `git add . && git commit -m "..." && git push origin main`
2. Pull on Oracle: `ssh oracle "cd cfb-tracker && git pull origin main"`
3. Rebuild/restart: `ssh oracle "cd cfb-tracker && docker compose up -d --build"`
4. Verify:
   ```bash
...
ssh oracle "cd cfb-tracker && docker compose restart"          # restart, no rebuild
ssh oracle "cd cfb-tracker && docker compose down"             # stop
